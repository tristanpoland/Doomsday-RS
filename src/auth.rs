...
impl UserPassAuthProvider {
    ...
    pub fn from_config(properties: &HashMap<String, serde_yaml::Value>) -> crate::Result<Self> {
        ...
        tracing::info!("Configuring {} users for authentication", users_config.len());
        ...
        tracing::info!("Authentication configured: session_timeout={}min, refresh_on_use={}", 
            session_timeout_minutes, refresh_on_use);
...
impl AuthProvider for UserPassAuthProvider {
    async fn authenticate(&self, request: &AuthRequest) -> crate::Result<AuthResponse> {
        ...
        if !valid {
            tracing::warn!("Authentication failed: invalid password for user {}", request.username);
        ...
        tracing::info!("Authentication successful for user: {} (token expires: {})", 
            request.username, expires_at);
    ...
    async fn validate_token(&self, token: &str) -> crate::Result<bool> {
        ...
        if let Some(mut session) = self.sessions.get_mut(token) {
            ...
            if self.refresh_on_use {
                ...
                tracing::debug!("Token refreshed for user: {} (new expiry: {})", 
                    session.username, session.expires_at);
            }
